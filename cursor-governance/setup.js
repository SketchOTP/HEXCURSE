...
## How this system runs (agent-first)
HexCurse governance is **fully driven by the Cursor agent** knowing and following this file, **HEXCURSE/SESSION_START_PROMPT.md**, **.cursor/rules/**, and MCP tools. It does **not** run as a background service. **When to use it:** every implementation chat — paste the session-start prompt (or \`@\` the paths it uses) at the top so Taskmaster, memory, and sequencing run.
...
- jcodemunch          → **RULE 10:** After repomix, index the workspace (\`resolve_repo\` / \`index_folder\`); outlines, \`search_symbols\`, \`get_context_bundle\`, \`get_blast_radius\`, etc. Package **\`jcodemunch-mcp\`** (often spelled **jcodemuch**).
...
- agents-memory-updater → RULE 9: human request, governance touch, or transcript delta + debounce; optional **\`--learning-rollup\`**. **HEXCURSE/docs/CONTINUAL_LEARNING.md**.
...
- Large file end-to-end reads when Serena symbol lookup suffices
...
### Session-close requirement
State in the **final handoff** and **HEXCURSE/SESSION_LOG.md**: MCP tools used (and why); MCPs not used (explicit reason each).

## Continual learning (self-improve)
HexCurse **closes the loop** from chat history into durable behavior: **memory MCP** plus **Learned Workspace Facts** in this file, via an **incremental transcript index**. Procedure: **HEXCURSE/docs/CONTINUAL_LEARNING.md** (paths in **HEXCURSE/PATHS.json**).
...
- **On session close:** If this session changed **governance** (rules, AGENTS, prompts, installer, PATHS, DIRECTIVES, continual-learning docs), run **agents-memory-updater** once before final handoff unless it already ran or the human skipped it.

## NORTH STAR → Taskmaster (one human step + one Cursor message)

- **Primary flow:** The human fills **\`HEXCURSE/NORTH_STAR.md\`**, then either (1) runs the **Cursor headless CLI** from repo root with **\`agent -p --model composer-2 --trust --workspace .\`** and the prompt in **\`HEXCURSE/HEADLESS_KICKOFF.txt\`** (see **\`HEXCURSE/ONE_PROMPT.md\`**), or (2) pastes **only** the fenced block from **\`HEXCURSE/ONE_PROMPT.md\`** as the **entire** first message in a new **in-IDE Agent** chat. Both instruct you to run the bridge via terminal using **\`.cursor/hexcurse-installer.path\`** (absolute path to **\`setup.js\`**), then run **SESSION START** from STEP 1 below.
...
## SESSION START — Execute this sequence in order. Do not skip steps. Do not reorder.
  ...
  STEP 0. **Only when the human did not use ONE_PROMPT.md but asked to run HEXCURSE / refresh from NORTH STAR:** run the CLI bridge from repo root (use **\`.cursor/hexcurse-installer.path\`**), then continue with STEP 1. If **\`HEXCURSE/NORTH_STAR.md\`** (or legacy repo-root **\`NORTH_STAR.md\`**) still has a standalone line that is only **\`NORTH_STAR_NOT_READY\`**, stop.
  ...
  STEP 3. Read HEXCURSE/DIRECTIVES.md. Confirm it matches Taskmaster state.
...
## DURING IMPLEMENTATION — Hard rules, not guidelines.
  ...
  - If a new idea occurs mid-session: add it to HEXCURSE/DIRECTIVES.md Backlog.
  ...
  - Update HEXCURSE/DIRECTIVES.md: add to the In Progress directive's Blockers field.
...
## SESSION CLOSE — Execute this sequence in order. Do not skip steps.
  ...
  STEP 5. Update HEXCURSE/DIRECTIVES.md: move completed directive to ✅ Completed
          with today's date and git short hash.

  STEP 6. Write HEXCURSE/SESSION_LOG.md entry using the template at the bottom of this file.
  ...
  STEP 10. Continual learning (self-improve): Run **agents-memory-updater** once per **HEXCURSE/docs/CONTINUAL_LEARNING.md**
