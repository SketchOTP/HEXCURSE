...
function agentsMd(projectName) {
  return `# AGENTS.md — AI Coder Rules of Engagement
# ${projectName}
...
## MCP Tools — Automatic Behavior (see \`mcp-usage.mdc\` and \`HEXCURSE/docs/MCP_COORDINATION.md\`)
...
- **Closing a session with Linear issues out of sync with Taskmaster** when Linear is in use.
- **Memory overriding** repo truth (Taskmaster, **HEXCURSE/DIRECTIVES.md**, **HEXCURSE/docs/ARCHITECTURE.md**, live tree beat stale memory).

### Session-close requirement
State in the **final handoff** and **HEXCURSE/SESSION_LOG.md**: MCP tools used (and why); MCPs not used (explicit reason each). Follow **SESSION CLOSE** below.

## Continual learning (self-improve)
HexCurse **closes the loop** from chat history into durable behavior: **memory MCP** plus **Learned Workspace Facts** in this file, via an **incremental transcript index**. Procedure: **HEXCURSE/docs/CONTINUAL_LEARNING.md** (index: **\`.cursor/hooks/state/continual-learning-index.json\`**).
...
**STEP 2.** Read **\`HEXCURSE/docs/ROLLING_CONTEXT.md\`** when the file exists. In a consumer install, the same file is **\`HEXCURSE/docs/ROLLING_CONTEXT.md\`**.

**STEP 3.** Call Taskmaster **get_tasks**. Identify the active task and next queued task. Report: \`Active: D[NNN] — [title]. Next queued: D[NNN] — [title].\` Then read **HEXCURSE/DIRECTIVES.md** (or **\`HEXCURSE/DIRECTIVES.md\`**) — confirm it matches Taskmaster. If out of sync, report and do not proceed until resolved.
...
**When making a significant architectural decision:** append an ADR to **\`HEXCURSE/docs/ADR_LOG.md\`** immediately (**adr.mdc**).

**When context window reaches ~70%:** write a **\`## COMPACTION CHECKPOINT\`** to **HEXCURSE/SESSION_LOG.md** (**memory-management.mdc**); prune stale tool output.
...
**STEP 4.** Run **semgrep** **security_check** on **all** source files modified this session. **Do not** close with unresolved **HIGH/CRITICAL** findings. If **semgrep** is unavailable, note the exception in **HEXCURSE/SESSION_LOG.md** and in the handoff (**process-gates.mdc**).
...
**STEP 7.** **ADR check:** If this session made a significant architectural decision, verify an entry in **\`HEXCURSE/docs/ADR_LOG.md\`**; write it now if missing.

**STEP 8.** Query **memory** — confirm discoveries were saved; write any missed.

**STEP 9.** Update **HEXCURSE/DIRECTIVES.md**: move completed directive to **Completed** with date and git short hash.

**STEP 10.** Write **HEXCURSE/SESSION_LOG.md** using the template below; include MCP utilization (used / not used + reasons).
...
**STEP 13.** **agents-memory-updater** per **RULE 9** / **HEXCURSE/docs/CONTINUAL_LEARNING.md**. Optionally **\`node cursor-governance/setup.js --learning-rollup\`** if **lastRollupAt** is stale or **5+** sessions since rollup.
...
- **MCP token budget:** each active server adds ~**500–1000** tokens of tool overhead. See **\`HEXCURSE/docs/MCP_TOKEN_BUDGET.md\`**. Disable session-conditional servers in **\`~/.cursor/mcp.json\`** when you will not use them; keep the **core** always-on set aligned with that doc.
...
*Classify new bullets per **\`HEXCURSE/docs/MEMORY_TAXONOMY.md\`**; merge into the subsection below. **agents-memory-updater** must not paste raw chat here.*
...
- **This repository** (HexCurse / cursor-governance **source**) keeps governance at **repo root** (\`AGENTS.md\`, \`DIRECTIVES.md\`, \`HEXCURSE/docs/\`, \`.cursor/rules/\`). Do **not** add a nested **\`HEXCURSE/\`** pack here (no **\`HexCurse/HEXCURSE/\`**); **\`HEXCURSE/docs/\`** + root files are canonical for this tree. **\`cursor-governance/setup.js\`** emits exactly **one** **\`HEXCURSE/\`** folder only when run **in other projects**.
- **\`HEXCURSE/docs/ARCH_PROMPT.md\`** is pasted into a **separate** Architect AI that **cannot see this repo**; it outputs **prompts for Cursor**. **HexCurse** is the governance layer **in Cursor**, not the product unless **HEXCURSE/docs/ARCHITECTURE.md** says otherwise.
- Generated Serena MCP config uses **\`--project\`** with the workspace folder, not **\`--project-root\`**.
- **jcodemunch** (**\`jcodemunch-mcp\`**, MCP id often **\`jcodemunch\`**) provides tree-sitter indexing and token-efficient retrieval; **Serena** remains mandatory for symbol-targeted **edits**. Session order and RULE 10: **\`mcp-usage.mdc\`** and **\`HEXCURSE/docs/MCP_COORDINATION.md\`**.
...
- **Unprimed chats** often skip Taskmaster and MCP sequencing despite rules — always paste **\`HEXCURSE/docs/SESSION_START_PROMPT.md\`** (or \`@\` its paths) at session start.
...
- **Prime every implementation chat** with **\`HEXCURSE/docs/SESSION_START_PROMPT.md\`** (or \`@\` the paths it lists) before substantive work.
...
function sessionStartMd(projectName) {
  return `# SESSION START PROMPT — ${projectName}
# **Fastest first-time kickoff:** Fill **HEXCURSE/NORTH_STAR.md**, then either run **Cursor headless** per **HEXCURSE/ONE_PROMPT.md** (\`agent -p --model composer-2\` + **HEXCURSE/HEADLESS_KICKOFF.txt**) **or** open **HEXCURSE/ONE_PROMPT.md** and paste **only** the in-IDE fenced block as your **entire** first Agent message — it runs the bridge CLI and then SESSION START automatically.
# **Every session after that:** Paste everything below the line into a new Cursor chat (or use ONE_PROMPT / headless again after editing NORTH_STAR).
# Edit the carry-over line at the bottom before pasting.
# Agent-first: paste this block every session — it makes the agent aware of the workflow (rules alone are not enough).
# ─────────────────────────────────────────────────────────────────────────────

@HEXCURSE/CURSOR.md @HEXCURSE/NORTH_STAR.md @HEXCURSE/PATHS.json @HEXCURSE/docs/MCP_COORDINATION.md @HEXCURSE/docs/GOVERNANCE_PARITY.md @HEXCURSE/docs/CONTINUAL_LEARNING.md @HEXCURSE/docs/CURSOR_MODES.md @HEXCURSE/AGENTS.md @HEXCURSE/DIRECTIVES.md @HEXCURSE/docs/ARCHITECTURE.md

You are the implementation agent for ${projectName}.

Execute the **SESSION START** sequence from **HEXCURSE/AGENTS.md** **now**, in order, without skipping steps and without waiting to be asked. The following steps **must match** the canonical text in **HEXCURSE/AGENTS.md** (including **4a–4e**):

**STEP 0 (bridge — when applicable).** Only if the human did **not** paste ONE_PROMPT but asked to run HEXCURSE / refresh from NORTH_STAR: use **\`.cursor/hexcurse-installer.path\`**, run **\`node …/setup.js --run-hexcurse\`** (or **\`--run-hexcurse-raw\`**), then continue at STEP 1 below. If **\`HEXCURSE/NORTH_STAR.md\`** (or legacy repo-root **\`NORTH_STAR.md\`**) still has a **standalone** line that is only **\`NORTH_STAR_NOT_READY\`**, stop.

**STEP 1.** Read **\`HEXCURSE/NORTH_STAR.md\`** or legacy repo-root **\`NORTH_STAR.md\`** when present.

**STEP 2.** Read **\`HEXCURSE/docs/ROLLING_CONTEXT.md\`** when the file exists.

**STEP 3.** Call Taskmaster **get_tasks**. Identify the active task and next queued task. Report: \`Active: D[NNN] — [title]. Next queued: D[NNN] — [title].\` Then read **HEXCURSE/DIRECTIVES.md** — confirm it matches Taskmaster. If out of sync, report and do not proceed until resolved.
...
**STEP 9.** **After** confirmation: create **local** branch **\`D[NNN]-[kebab-desc]\`** with **git** before the first line of code, unless the human says the branch already exists. Do **not** use **github** MCP unless the human explicitly asked for a remote branch via API.

**STEP 10.** Begin implementation (see **DURING IMPLEMENTATION** in **HEXCURSE/AGENTS.md**).
...
[REPLACE THIS LINE: add carry-over context from last session, or delete this line if starting fresh]
