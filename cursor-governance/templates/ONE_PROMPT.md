...
From the **repository root**, the prompt body lives in **`HEXCURSE/HEADLESS_KICKOFF.txt`** (plain text; same steps as the in-IDE block, without `@` mentions).

**macOS / Linux (bash):**

```bash
export CURSOR_API_KEY=your_key_here   # if required
agent -p --model composer-2 --trust --workspace "$(pwd)" "$(tr -d '\r' < HEXCURSE/HEADLESS_KICKOFF.txt)"
```
...
agent -p --model composer-2 --trust --workspace $PWD (Get-Content -Raw .\HEXCURSE\HEADLESS_KICKOFF.txt)
```
...
**No AI for the bridge?** Run in terminal yourself: `node "<path-to>/setup.js" --run-hexcurse-raw` then paste **HEXCURSE/SESSION_START_PROMPT.md** in a new chat (or use headless with the same **`HEXCURSE/HEADLESS_KICKOFF.txt`** body after editing step A manually).
