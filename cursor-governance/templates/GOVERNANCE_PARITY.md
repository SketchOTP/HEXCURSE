# Governance parity — {{PROJECT_NAME}}
...
| **`HEXCURSE/AGENTS.md`** | Session ritual and close checklist; procedural contract. |
| **`HEXCURSE/docs/MCP_COORDINATION.md`** | Human-readable map; if it disagrees with `.mdc` rules, **rules win**. |

## What does *not* run by itself

- **MCP tools** are **not** invoked by the filesystem or `alwaysApply` rules. The agent must call them; the human must keep MCP servers **green** in Cursor.
- **Session start** (`HEXCURSE/SESSION_START_PROMPT.md`) must be **pasted** (or `@` files) so the agent runs memory → Taskmaster → repomix → **jcodemunch** index/outline → sequential-thinking in order.
...
- **CI** (if configured): tests + doctor in **CI mode** (machine-local MCP / `task-master` checks are non-blocking on generic runners).

## Continual learning (RULE 9)

Per **`HEXCURSE/docs/CONTINUAL_LEARNING.md`**. Does **not** run on every chat automatically.

## Appendix — AGENTS.md steps vs automation (lightweight parity)

Only **loading** `.cursor/rules/*.mdc` is automatic when Cursor opens a chat; **nothing in the SESSION START / CLOSE sequences runs without the agent or human.**
...
| 10 — RULE 9 / continual learning | Yes (agents-memory-updater Task) | Optional (`setup.js --learning-rollup`) | Per **HEXCURSE/docs/CONTINUAL_LEARNING.md** |

**Doctor:** **`--doctor`** prints a **template fingerprint** and **`HEXCURSE/docs/`** file count when the consumer pack and installer **`templates/`** are both available.
